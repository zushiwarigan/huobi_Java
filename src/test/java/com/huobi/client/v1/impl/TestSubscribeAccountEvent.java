...
import com.huobi.client.v1.model.User;
import com.huobi.client.v1.enums.AccountChangeType;
...
public class TestSubscribeAccountEvent {
  ...
  public void testResult() {
    ...
    assertEquals(1, event.getAccountChangeList().size());
    assertEquals(AccountChangeType.NEWORDER, event.getChangeType());
    assertEquals(AccountType.SPOT, event.getAccountChangeList().get(0).getAccountType());
    assertEquals("ht", event.getAccountChangeList().get(0).getCurrency());
    assertEquals(new BigDecimal("10.8208984536412"), event.getAccountChangeList().get(0).getBalance());
    assertEquals(BalanceType.TRADE, event.getAccountChangeList().get(0).getBalanceType());
  }
}
